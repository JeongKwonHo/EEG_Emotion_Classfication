...
for k = 1:8
    ...
    for j = 1:18        % 영상 18개
        ...
        if j == 1 || j == 9 || j == 12 || j == 7 || j == 14 || j == 17 || j == 3 || j == 5 || j == 15 % 경험이니까 버려
            continue
        end
        newArray = zeros(1, 123);
        for i = 4:11    %EEG 8개
            ...
            for N = 1:5
                if N == 1
                    newArray(1, ((i-4)*15) + 1) = delta_mean(j,i-3);
                    newArray(1, ((i-4)*15) + 2) = delta_std(j,i-3);
                    newArray(1, ((i-4)*15) + 3) = delta_var(j,i-3);
                elseif N == 2
                    newArray(1, ((i-4)*15) + 4) = theta_mean(j,i-3);
                    newArray(1, ((i-4)*15) + 5) = theta_std(j,i-3);
                    newArray(1, ((i-4)*15) + 6) = theta_var(j,i-3);
                elseif N == 3
                    newArray(1, ((i-4)*15) + 7) = alpha_mean(j,i-3);
                    newArray(1, ((i-4)*15) + 8) = alpha_std(j,i-3);
                    newArray(1, ((i-4)*15) + 9) = alpha_var(j,i-3);
                elseif N == 4
                    newArray(1, ((i-4)*15) + 10) = beta_mean(j,i-3);
                    newArray(1, ((i-4)*15) + 11) = beta_std(j,i-3);
                    newArray(1, ((i-4)*15) + 12) = beta_var(j,i-3);
                elseif N == 5
                    newArray(1, ((i-4)*15) + 13) = gamma_mean(j,i-3);
                    newArray(1, ((i-4)*15) + 14) = gamma_std(j,i-3);
                    newArray(1, ((i-4)*15) + 15) = gamma_var(j,i-3);
                end
            end
            newArray(1, 121) = 5;
            newArray(1, 122) = 4;
        ...
        if j == 4 || j == 13 || j == 16
            cell1(a, 1:123) = newArray(1, 1:123);
            a = a + 1;
        elseif j == 2 || j == 6 || j == 10
            cell2(b, 1:123) = newArray(1, 1:123);
            b = b + 1;
        elseif j == 8 || j == 11 || j == 18
            cell3(c, 1:123) = newArray(1, 1:123);
            c = c + 1;
        end
    end
end
FINAL_DATA{1,1} = cell1;
FINAL_DATA{1,2} = cell2;
FINAL_DATA{1,3} = cell3;
